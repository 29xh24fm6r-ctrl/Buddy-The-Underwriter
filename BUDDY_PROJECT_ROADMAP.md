...
Remaining open tickets from the original spec, in updated priority order: **principal residence certification** (P0) → **live Plaid Link for the debt-schedule auto-builder** (P0, credential-gated) → Ticket 2 (identity/e-sign) → Ticket 4 (score framing, needs Matt's decision) → Ticket 5 (XLSX tables) → Ticket 6 (lender blind-review doc, needs Matt's decision) → Ticket 8 (end-to-end synthetic run, closing ticket). Ticket 7 (marketplace liquidity) is explicitly not a code ticket. Separately open, flagged this pass: wholesale migration of the SBA forward model's projection math onto finengine's `CashFlowMethod`/`projectionEngine` interfaces — a larger architectural project, not started.

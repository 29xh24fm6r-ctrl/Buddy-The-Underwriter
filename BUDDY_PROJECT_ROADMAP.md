...
2. **Fix A — Route Consolidation** (immediate precondition for FIX-C enforcement). Scoped audit to pull route count from ~2053 down to ≤2010 with real cushion. Candidate targets (to be confirmed in Fix A's own spec): the 13 `/api/debug/*` diagnostic routes, the `/admin/demo/*` tree, remaining `/status` sibling directories beyond the two collapsed in D5's rescue fix, and any orphan `route.ts` files with zero callers in `src/`. Once Fix A lands under threshold, a trivial third PR flips `.github/workflows/route-budget.yml` from advisory to enforcing (exit 1 at status=error). Until Fix A ships, FIX-C is measurement-only — the tool posts counts and deltas on every PR but never blocks a merge.
3. **Phase 84.1 backlog** — see `docs/archive/phase-84/` for generated tickets, including the gating **T-08-G** (production activity baseline). Until T-08-G is answered ("are there any live paying banks?"), the rest of 84.1 priority ordering is provisional. Tickets queued: T-08-A (recon rarity), T-08-C (executeCanonicalAction never invoked), T-08-E (wire analyst-correction UI), T-08-B/D/F (governance writers), RLS Batch B, fact re-parenting, `.update()/.insert()` silent-error audit, **D2 — Omega MCP `Method not found` investigation (Buddy-side correct; Omega/Pulse side needs audit of exposed methods vs. what Buddy calls)**.

...
**Environmental blockers (not code gaps — see Drift Log for full list):** no live SBA/IRS network access, no Persona/DocuSeal/CAIVRS/SAM/E-Tran vendor credentials provisioned in this environment, no GCP/Cloud Run access for DocuSeal deployment, no fully-populated SBA smoke deal in prod for live end-to-end verification. `deal_truth_snapshots`' schema drift (an untracked migration had silently stubbed it down to 3 columns) was root-caused and fixed 2026-07-18 — see Drift Log — but the truth-snapshot *writer* subsystem it feeds (`arbitration_decisions`, `bank_overlays` — neither table exists in this database) is still a separate, larger, unbuilt effort, so `generateETranXML` still has no real data to read for any deal today.

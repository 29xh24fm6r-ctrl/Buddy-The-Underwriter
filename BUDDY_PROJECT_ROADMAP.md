...
**Environmental blockers (not code gaps — see Drift Log for full list):** no live SBA/IRS network access, no Persona/DocuSeal/CAIVRS/SAM/E-Tran vendor credentials provisioned in this environment, no GCP/Cloud Run access for DocuSeal deployment, no fully-populated SBA smoke deal in prod for live end-to-end verification, and **no Clerk credentials configured at all in this environment** (confirmed by actually starting the dev server — every auth-gated route fails at `getCurrentBankId()` before reaching business logic), which blocked a full HTTP-level integration test of the autopilot pipeline fix below. `deal_truth_snapshots`' schema drift, the truth-snapshot *writer* subsystem feeding it (6 tables), and — as of 2026-07-19 — `deal_pipeline_runs` (the autopilot pipeline's own run-tracker, also silently dropped) were all root-caused and restored; see Drift Log for each investigation and its live smoke test. The 2026-07-19 fix also found and fixed a second, independent, compounding bug: the pipeline's own S2-S6 stages called sibling API routes via unauthenticated server-to-server `fetch()`, which would have failed every real run even with the schema fixed — refactored to in-process calls. The "Make E-Tran Ready" trigger now lives on the deal Cockpit page as a real, working panel (previously the button's route was a decoy and every UI caller was orphaned). Two things this fix did *not* reach, flagged rather than silently left implicit: (1) 5 of the 9 agents the pipeline invokes (`credit`/`collateral`/`management`/`narrative`/`evidence`) still have no implementation — the pipeline now reports this honestly instead of claiming full success; (2) S1's connected-account document-substitution step depends on 3 more missing tables (`document_substitutions`/`connected_account_data`/`borrower_account_connections`) — happens to degrade gracefully today (the code never checks that query's error field), but that feature doesn't actually function.
...
**Ported Brokerage identity/e-sign onto Didit/SignWell before merging to main — closed 2026-07-16.** While this branch was in flight, `main` independently merged commit `396104a0` (PR #701), swapping the Underwriter tenant's identity-verification vendor (Persona → Didit) and e-signature vendor (DocuSeal → SignWell) — a real interface change to the shared `kyc/service.ts`/e-sign service layer, not a text conflict. This branch's entire Brokerage identity/e-sign build (Ticket 2, the mock-vendor harness, `SigningPanel`) was written against the old interfaces. Researched main's new shapes fully before touching anything, then ported: `mockPersona.ts` → new `mockDidit.ts` (simpler — Didit's session-create response already returns a usable URL, no separate one-time-link round trip); `esign/docuseal/mock*.ts` moved to `esign/signwell/` and rewritten against `SignwellClient` (one fewer method — SignWell embeds its audit trail in the PDF itself, no separate artifact); the Brokerage `borrower-actions` route repointed at Didit/SignWell deps and the renamed `signed_documents` columns. The external API shape (`oneTimeLink`, `submission_id`, `embed_url`) was unchanged by main's swap, so `IdentityVerificationCard.tsx`/`SigningPanel.tsx` needed zero changes. Deleted an obsolete `mock_persona` vendor-CHECK migration, replaced with one adding `mock_didit` to the constraint's current value list. One real test bug found and fixed while porting (not just a rename): an integration-test assertion expected the literal string `"kyc.verification_completed"`, but Didit's `"Approved"` status maps to Buddy's `"approved"` — a different, equally-valid terminal-success string. `tsc` clean, full `pnpm test:unit` 11,643 passed on the fully merged tree. Full writeup: `docs/archive/brokerage-sba-ready-v1/T11-didit-signwell-port-AAR.md`.

Remaining open tickets from the original spec, in updated priority order: **live Plaid Link for the debt-schedule auto-builder** (P0, credential-gated — blocked, no vendor credentials in this environment) → official SBA/IRS PDF template ingestion (blocked on this environment's network policy — user is updating it directly; once done, both the one-off ingestion script and the new recurring staleness checker work unattended, no further code needed) → real Didit/SignWell vendor provisioning (needed once the mock harness's job — proving the flow works — is done and this needs to go live) → Ticket 4's framing decision (Matt) → Ticket 6 (lender blind-review doc, needs Matt's decision — explicitly "decide, not build") → Ticket 8 (end-to-end synthetic run, closing ticket — now unblocked for testing via the mock harness, still blocked for a real production run on the same vendor credentials as Plaid/Didit/SignWell). Ticket 7 (marketplace liquidity) is explicitly not a code ticket. Separately open, flagged during the finengine pass: wholesale migration of the SBA forward model's projection math onto finengine's `CashFlowMethod`/`projectionEngine` interfaces — a larger architectural project, not started.

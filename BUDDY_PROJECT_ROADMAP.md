...
Remaining open tickets from the original spec, in updated priority order: **live Plaid Link for the debt-schedule auto-builder** (P0, credential-gated — blocked, no vendor credentials in this environment) → Brokerage per-owner SBA form generation (new, surfaced by Ticket 2 — needed before the e-signature UI can be built) → Ticket 4's framing decision (Matt) → Ticket 6 (lender blind-review doc, needs Matt's decision — explicitly "decide, not build") → Ticket 8 (end-to-end synthetic run, closing ticket — blocked on the same vendor credentials as Plaid). Ticket 7 (marketplace liquidity) is explicitly not a code ticket. Separately open, flagged during the finengine pass: wholesale migration of the SBA forward model's projection math onto finengine's `CashFlowMethod`/`projectionEngine` interfaces — a larger architectural project, not started.

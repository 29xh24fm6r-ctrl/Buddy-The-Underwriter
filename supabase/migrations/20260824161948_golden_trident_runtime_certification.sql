...
-- Golden Trident Runtime Certification Factory
-- Expired workflow leases become terminal, observable failures instead of
-- remaining permanently visible as active work. The existing five-minute
-- lock janitor invokes this service-role-only function.

create index if not exists buddy_trident_bundles_stale_lease_idx
  on public.buddy_trident_bundles (lease_expires_at, id)
...
begin
  ...
  recorded as (
    ...
    select
      ...
      v_now
    from expired e
    on conflict (bundle_id, stage) do update
      set status = 'failed',
          error_json = coalesce(

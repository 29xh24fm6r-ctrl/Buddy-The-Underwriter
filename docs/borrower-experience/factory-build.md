...
- Five short missions organize the existing questions: Shape your idea, Tell your story, Build your budget, Prepare your package and Make it ready. The complete applicable application map remains available. Optional unrelated topic prompts can be deferred without hiding required fields or saved answers. Each mission names the useful artifact it unlocks instead of awarding points or implying creditworthiness.
- A live project card assembles only confirmed application facts (business, goal, location, timeline and funds needed). Unknown facts stay visibly unknown. A successful answer write now advances to the next relevant question and produces restrained milestone feedback; a failed write leaves the draft and position intact.

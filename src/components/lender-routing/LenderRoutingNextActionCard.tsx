...
import type { LenderRoutingNextAction } from "@/lib/banker/buildLenderRoutingFitViewModel";

const URGENCY_STYLES: Record<
  LenderRoutingNextAction["urgency"],
...
export function LenderRoutingNextActionCard({
  action,
}: {
  action: LenderRoutingNextAction;
}) {
  const style = URGENCY_STYLES[action.urgency];
  return (
    <section
      role="region"
      aria-label="Lender routing next action"
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-white">Next routing action</h3>
        <span
          className={cn(

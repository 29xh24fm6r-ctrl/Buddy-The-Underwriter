...
function AppetiteSummary({ profile: p, expanded = false }: { profile: Json; expanded?: boolean }) {
  ...
  const rows = [["Programs", programs.join(", ") || "Not set"],["Loan size", p.min_loan_amount || p.max_loan_amount ? `${p.min_loan_amount ? fmtMoney(Number(p.min_loan_amount)) : "Any"} – ${p.max_loan_amount ? fmtMoney(Number(p.max_loan_amount)) : "Any"}` : "Not set"],["Credit box", [p.min_dscr && `DSCR ≥ ${p.min_dscr}`,p.max_ltv && `LTV ≤ ${p.max_ltv}%`,p.minimum_fico && `FICO ≥ ${p.minimum_fico}`].filter(Boolean).join(" · ") || "Not set"],["Geography",(p.geographies||[]).join(", ")||"Not set"],["Industries",(p.industries||[]).join(", ")||"Not set"],["Excluded",(p.excluded_industries||[]).join(", ")||"None"],["Response target",p.response_sla_days ? `${p.response_sla_days} days` : "Not set"],["Deal preferences",p.deal_preferences||"Not set"]];
...
function AppetiteForm({ value: v, setValue: set, busy, save, cancel }: any) {
  return <FormShell title="Define lending appetite" description="Capture the bank’s credit box once so every future deal can be matched and tracked consistently." save={save} cancel={cancel} busy={busy} saveLabel="Save lending appetite"><div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10 }}>
    {label("Relationship stage",<select style={field()} value={v.relationshipStatus} onChange={e=>set({...v,relationshipStatus:e.target.value})}><option value="prospect">Prospect</option><option value="active">Active</option><option value="preferred">Preferred</option><option value="paused">Paused</option><option value="inactive">Inactive</option></select>)}{label("Lender type",<select style={field()} value={v.lenderType} onChange={e=>set({...v,lenderType:e.target.value})}><option value="bank">Bank</option><option value="credit_union">Credit union</option><option value="non_bank">Non-bank lender</option><option value="fund">Debt fund</option><option value="other">Other</option></select>)}{label("Minimum loan",<input type="number" style={field()} value={v.minLoanAmount} onChange={e=>set({...v,minLoanAmount:e.target.value})}/>)}{label("Maximum loan",<input type="number" style={field()} value={v.maxLoanAmount} onChange={e=>set({...v,maxLoanAmount:e.target.value})}/>)}{label("Minimum DSCR",<input type="number" step=".01" style={field()} value={v.minDscr} onChange={e=>set({...v,minDscr:e.target.value})}/>)}{label("Maximum LTV %",<input type="number" style={field()} value={v.maxLtv} onChange={e=>set({...v,maxLtv:e.target.value})}/>)}{label("Minimum FICO",<input type="number" style={field()} value={v.minimumFico} onChange={e=>set({...v,minimumFico:e.target.value})}/>)}{label("Response target (days)",<input type="number" style={field()} value={v.responseSlaDays} onChange={e=>set({...v,responseSlaDays:e.target.value})}/>)}

// src/components/deals/BorrowerWowCard.tsx
...
export default function BorrowerWowCard({ borrower, className }: BorrowerWowCardProps) {
  ...
  const hasAnything = !!borrower.name || !!borrower.address?.raw || !!borrower.ein_last4;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end">
          ...
          <div className={["text-sm font-semibold", conf.tone].join(" ")}>{conf.label}</div>
        </div>
      </div>
      ...
      <div className="mt-3 space-y-2 text-sm">
        {borrower.address?.raw ? (
          <div className="rounded-lg bg-zinc-50 p-3 text-zinc-700 dark:bg-zinc-900/40 dark:text-zinc-200">
            ...
            <div className="mt-1 whitespace-pre-line leading-5">{borrower.address.raw}</div>
          </div>
        ) : null}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {ein ? (
            ...
            <div className="rounded-full border px-3 py-1 text-xs text-zinc-500">EIN: —</div>
          )}
          ...
          <div className="ml-auto text-xs text-zinc-500">(No schema changes)</div>
        </div>
      </div>
    </div>
  );

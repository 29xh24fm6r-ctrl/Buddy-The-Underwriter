...
// Phase 85A — 4-step mobile-first borrower intake form
//
...
const STEP_LABELS = ["Business Info", "Business Address", "Loan Request", "Review & Submit"];
...
export function IntakeFormClient({ token, dealId, deal, borrower, existingSections, existingApplication }: Props) {
  ...
  useEffect(() => {
    if (step === 3) debouncedSave("loan", loan as unknown as Record<string, unknown>);
  ...
  const goNext = useCallback(() => {
    ...
    if (step < 4) setStep((s) => (s + 1) as IntakeStep);
    setError(null);

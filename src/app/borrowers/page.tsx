...
export default async function BorrowersPage() {
  ...
  const borrowers = new Map<string, { name: string; dealId: string; dealCount: number }>();
    ...
    <div className="min-h-screen bg-[#0f1115] text-white">
      <header className="border-b border-white/10 bg-[#111418] px-6 py-4">
        <div>
          ...
          <p className="text-sm text-white/60 mt-1">Borrower directory across active deals</p>
        </div>
      </header>
        ...
        <div className="bg-[#181b21] border border-white/10 rounded-lg overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/10">
              {list.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-6 py-10 text-center text-white/50">
                    No borrowers found.
                  </td>
                </tr>
              )}

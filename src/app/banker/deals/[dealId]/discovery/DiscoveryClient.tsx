        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className={`${card()} p-4 min-w-[220px]`}>
            ...
            <div className="text-xl font-semibold mt-1">
              {Number(session?.completeness ?? 0).toFixed(0)}%
            </div>
            <div className="text-xs text-white/60 mt-2">
              Status: <span className="text-white/90">{session?.status || "—"}</span>
            </div>
            <div className="text-xs text-white/60 mt-1">
              Missing: {(session?.missing_domains || []).join(", ") || "None"}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
          <div className={`${card()} p-4 lg:col-span-2`}>
            ...
            <div className="text-white/60 text-sm mt-1">
              Structured facts (sources + confidence can be added to UI next).
            </div>
          ...
          <div className="space-y-4">
            <div className={`${card()} p-4`}>
              ...
              <div className="text-white/60 text-sm mt-1">
                Entities, edges, and derived requirements.
              </div>
            ...
            <div className={`${card()} p-4`}>
              ...
              <button
                className="mt-4 w-full px-4 py-2 rounded-xl border border-white/15 bg-white/5 transition text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={generateUwDraft}
                disabled={!underwriteAllowed}
                title={!underwriteAllowed ? "Complete intake before this action is available" : undefined}
              >

...
export async function POST(req: Request) {
  ...
  const link = await sb
    ...
    .select(
      "id, deal_id, expires_at, revoked_at, single_use, used_at, requested_keys",
    )
  ...
  if (checklist.error)
    return NextResponse.json(
      { ok: false, error: checklist.error.message },
      { status: 500 },
    );

  const items = checklist.data || [];
  const missing = items.filter(
    (x: any) => x.status === "missing" || x.status === "requested",
  );

  // If the link is scoped to requested_keys, only speak about those keys
  const requestedKeys: string[] = Array.isArray(link.data.requested_keys)
    ? link.data.requested_keys
    : [];

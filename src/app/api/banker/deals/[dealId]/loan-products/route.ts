...
export async function POST(
  ...
  try {
    ...
    if (body?.kind === "underwrite") {
      const row = await upsertUnderwriteInput({
        ...(body.data ?? {}),
        deal_id: dealId,
      });
    ...
    if (body?.kind === "copy_request_to_underwrite") {
      ...
      const draft = await upsertUnderwriteInput({
        ...
        proposed_interest_only_months:
          (src as any).requested_interest_only_months ?? null,
    ...
    throw new Error(
      "Invalid kind. Use kind=request, kind=underwrite, or kind=copy_request_to_underwrite.",
    );
  } catch (e: any) {
    return NextResponse.json(
      { ok: false, error: e?.message ?? "Unknown error" },

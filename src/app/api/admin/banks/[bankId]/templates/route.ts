...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ bankId: string }> }) {
  try {
    requireSuperAdmin();
  ...
  } catch (err: any) {    const a = authzError(err);
    if (a) return NextResponse.json(a.body, { status: a.status });    return NextResponse.json({ ok: false, error: err?.message ?? String(err) }, { status: 500 });
  }
}

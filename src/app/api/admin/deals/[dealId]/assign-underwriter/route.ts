...
import { registerUnderwriterParticipant, getDealParticipants } from "@/lib/deals/participants";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ dealId: string }> }) {
  ...
  try {
    ...
    await registerUnderwriterParticipant(dealId, clerk_user_id, actor ?? undefined);

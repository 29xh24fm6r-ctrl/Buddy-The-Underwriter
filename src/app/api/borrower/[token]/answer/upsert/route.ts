...
export async function POST(req: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (!question_key) {
      return NextResponse.json({ ok: false, error: "Missing question_key" }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: false, error: err?.message ?? "answer_upsert_failed" }, { status: 400 });
  }
}

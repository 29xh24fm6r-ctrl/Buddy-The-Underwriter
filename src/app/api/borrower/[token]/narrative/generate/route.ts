...
export async function POST(_: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    const [{ data: forms }, { data: preflight }, { data: reqs }, { data: elig }] =
      await Promise.all([
        ...
        (sb as any).from("sba_eligibility_snapshots").select("*").eq("application_id", application.id).order("created_at", { ascending: false }).limit(1).single(),
      ]);

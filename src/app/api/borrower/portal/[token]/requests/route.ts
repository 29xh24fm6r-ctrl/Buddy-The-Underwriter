...
export async function GET(
  ...
  try {
    ...
    const reqs = await sb
      ...
      .select(
        "id, title, category, description, required, status, due_at, received_filename, received_at, sort_order",
      )
    ...
    const { data: recentUploads } = await sb
      .from("borrower_upload_inbox")
      .select(
        "id, filename, created_at, matched_request_id, match_confidence, status",
      )
      .eq("deal_id", invite.deal_id)

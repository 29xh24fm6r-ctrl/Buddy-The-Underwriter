...
export async function POST(req: Request, { params }: { params: { dealId: string } }) {
  ...
  if (!condition_id) return NextResponse.json({ ok: false, error: "missing_condition_id" }, { status: 400 });
  if (!["open", "satisfied", "waived", "rejected"].includes(status)) return NextResponse.json({ ok: false, error: "invalid_status" }, { status: 400 });
  ...
  if (cur.error) return NextResponse.json({ ok: false, error: "condition_fetch_failed", detail: cur.error.message }, { status: 500 });
  if (!cur.data) return NextResponse.json({ ok: false, error: "condition_not_found" }, { status: 404 });
  ...
  if (String(cur.data.source) === "policy" && cur.data.source_key) {
    ...
    if (status === "satisfied") {
      await sb.from("deal_mitigants").update({
        status: "satisfied",
        satisfied_at: new Date().toISOString(),
        satisfied_by: auth.user.id,
        note,
      }).eq("deal_id", dealId).eq("mitigant_key", mitigant_key);

...
async function renderMoodysPdf(req: NextRequest, dealId: string) {
  ...
  const previewUrl = `${protocol}://${host}/deals/${dealId}/spreads/moodys/print?token=${encodeURIComponent(token)}`;
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      headerTemplate: `
        <div style="font-size:8px; width:100%; padding:0 0.5in; color:#999; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial;">
          <span>Moody's Financial Analysis • Institutional Spread Package</span>
        </div>
      `,
      footerTemplate: `
        <div style="font-size:8px; width:100%; padding:0 0.5in; color:#999; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial;">
    ...
    return new NextResponse(pdfBlob, {
      ...
      headers: {
        ...
        "content-disposition": `inline; filename="moodys-financial-analysis-${dealId}.pdf"`,
      },
    });
  } finally {
    await browser.close();
...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  return renderMoodysPdf(req, dealId);
}

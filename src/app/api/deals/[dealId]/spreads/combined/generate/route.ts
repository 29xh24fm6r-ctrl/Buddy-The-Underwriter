...
export async function POST(req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!entity_ids || !Array.isArray(entity_ids) || entity_ids.length === 0) {
      return json(400, { ok: false, error: "Missing or invalid entity_ids array" });
    }
    ...
    const combinedSpread = {
      ...
      deal_id: dealId,
      user_id: 'dev-user',
      scope: entity_ids.length === selectedEntities.filter(e => e.entity_kind !== 'GROUP').length ? 'GROUP' : 'SELECTED',
      entity_ids,
      period_type,
      fiscal_year,
      currency: 'USD',

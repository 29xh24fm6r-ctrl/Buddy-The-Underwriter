...
export async function GET(
  ...
  try {
    ...
    const outstanding = (conds ?? []).filter(
      (c: any) => c.status !== "satisfied",
    );
    const conditionsOutstanding = outstanding.length;
    const conditionsCritical = outstanding.filter(
      (c: any) => c.severity === "CRITICAL",
    ).length;
    const conditionsHigh = outstanding.filter(
    ...
    const draftMessages = (msgs ?? []).filter(
      (m: any) => m.status === "DRAFT",
    ).length;
    ...
    const eligibleUploads = (atts ?? []).filter((a: any) =>
      OCR_OK.has(a.mime_type),
    ).length;
    ...
    const ocrCompletedCount = new Set(
      (ocrRes ?? []).map((r: any) => r.attachment_id),
    ).size;
    ...
    const formsReadyToGenerate = (fillRuns ?? []).filter(

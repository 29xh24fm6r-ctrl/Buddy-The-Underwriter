...
function buildNextActions(results: PolicyEvaluationResult["results"]) {
  ...
  const map = new Map<string, { key: string; label: string; priority: number; reason_rule_keys: string[] }>();
  ...
  for (const r of results) {
    ...
    for (const m of r.mitigants || []) {
      ...
      if (!existing) {
        map.set(m.key, { key: m.key, label: m.label, priority: pri, reason_rule_keys: [r.rule_key] });
      } else {
        existing.priority = Math.min(existing.priority, pri);
        if (!existing.reason_rule_keys.includes(r.rule_key)) existing.reason_rule_keys.push(r.rule_key);
      }
    }
  }

  return Array.from(map.values()).sort((a, b) => a.priority - b.priority).slice(0, 12);
}
...
export async function POST(req: Request, { params }: { params: { dealId: string } }) {
  ...
  const rulesRes = await sb
    ...
    .select("id,bank_id,rule_key,title,description,scope,predicate,decision,mitigants,exception_template,severity,active")
    .eq("bank_id", bankId)
    .eq("active", true)
    .order("severity", { ascending: true });

  if (rulesRes.error) return NextResponse.json({ ok: false, error: "rules_fetch_failed", detail: rulesRes.error.message }, { status: 500 });

  const rules = (rulesRes.data ?? []) as any[];

  const citRes = await sb
    .from("bank_policy_rule_citations")
    .select("rule_id, asset_id, chunk_id, note, bank_policy_chunks:chunk_id(content,page_num,section)")
    .eq("bank_id", bankId);
  ...
  const results = evaluateRules(rules, ctx, evidenceByRuleId);
  ...
  const mitigants_total = results.reduce((acc, r) => acc + (r.mitigants?.length || 0), 0);

...
/**
 * GET /api/deals/[dealId]/reclassify-all
 *
 * Preflight summary for the POST above. Reports:
 ...
 *
 * Merged into the same file as POST to stay under Vercel's 2048-route cap.
 * Previously lived at /reclassify-all/status — the client now GETs this path
 * instead.
 */
export async function GET(_req: NextRequest, ctx: Ctx) {
  try {
    ...
    console.error("[reclassify-all GET] error", error);
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Internal server error",

...
export async function GET(_req: Request, ctx: { params: Promise<{ dealId: string }> }) {
  ...
  const { data: uploads, error } = await sb
    ...
    .select("id,deal_id,bank_id,request_id,original_filename,mime_type,size_bytes,storage_bucket,storage_path,uploaded_at")
    .eq("deal_id", dealId)
    .is("request_id", null)
    .order("uploaded_at", { ascending: false })
    .limit(100);

  if (error) return NextResponse.json({ error: "Failed to load uploads" }, { status: 500 });

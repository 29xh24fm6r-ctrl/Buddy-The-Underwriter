...
export async function POST(req: Request, { params }: { params: { dealId: string } }) {
  ...
  if (!storedName) {
    return NextResponse.json({ ok: false, error: "Missing stored_name (storage path)." }, { status: 400 });
  }
  ...
  if (!doc) {
    return NextResponse.json({ ok: false, error: "File not found for this deal." }, { status: 404 });
  }

...
/**
 * PATCH /api/deals/[dealId]/research/committee-tasks/[taskId]/review
 * SPEC-BIE-COMMITTEE-EVIDENCE-REVIEW-ACTIONS-1
 *
 * Apply a banker/analyst review action to one committee evidence task. Writes
 * durable review state on the task and an append-only audit row. NEVER changes
 * gate scoring, trust grade, preliminary/committee eligibility, or clears a
 * committee blocker.
 *
 * Body: { action, note?, reason? }
 * Response: { ok: true, task, review }
 *
 * mission_id / deal_id for the audit row come from the trusted DB row, never the
 * client. The task is verified to belong to the URL dealId.
 */
export async function PATCH(req: NextRequest, ctx: { params: Params }) {
  try {
    ...
    if (auditErr) {
      // The task state is already persisted; surface the audit failure but don't
      // pretend the review didn't happen.
      return NextResponse.json(

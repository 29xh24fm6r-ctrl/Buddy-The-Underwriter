...
export async function POST(req: Request, ctx: { params: Promise<{ dealId: string }> }) {
  ...
  const startInHours = Math.max(0, Math.min(24 * 30, Number(body.startInHours ?? 24)));
  ...
  if (exErr) {
    return NextResponse.json(
      { ok: false, error: "Failed to check existing reminder subscription.", detail: exErr.message },
      { status: 500 }
    );
  }
  ...
  if (existing?.id) {
    ...
    if (upErr) {
      return NextResponse.json(
        { ok: false, error: "Failed to update reminder subscription.", detail: upErr.message },
        { status: 500 }
      );
    }
  ...
  if (insErr || !inserted) {
    return NextResponse.json(
      { ok: false, error: "Failed to create reminder subscription.", detail: insErr?.message },
      { status: 500 }
    );
  }

...
export async function POST(
  ...
  try {
    ...
    const packageTemplateCode =
      (body?.packageTemplateCode as string) ?? "SBA_7A_BASE";
    const product = (body?.product as "7a" | "504" | "express") ?? "7a";

    const answers = (body?.answers ?? {}) as Record<string, any>;
    const borrowerData = (body?.borrowerData ?? null) as Record<
      string,
      any
    > | null;
    ...
    return NextResponse.json(
      { ok: false, error: e?.message || "sba_package_prepare_failed" },
      { status: 500 },
    );
  }
}

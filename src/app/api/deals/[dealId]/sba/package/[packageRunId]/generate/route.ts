...
export async function POST(req: Request, ctx: { params: Promise<{ dealId: string; packageRunId: string }> }) {
  try {
    ...
    const list = (items ?? []).filter((it: any) => !onlyItemId || it.id === onlyItemId);
    ...
    for (const it of list) {
      ...
      if (!fillRunId) {
        await supabase.from("sba_package_run_items").update({ status: "failed", error: "Missing fill_run_id" }).eq("id", it.id);
      ...
      try {
        ...
        await supabase.from("sba_package_run_items").update({
          status: "generated",
        ...
        await supabase.from("sba_package_run_items").update({ status: "failed", error: e?.message || "generate_failed" }).eq("id", it.id);
        results.push({ itemId: it.id, ok: false, error: e?.message || "generate_failed" });
      }
    }

    const anyFailed = results.some((r) => !r.ok);
    await supabase.from("sba_package_runs").update({ status: anyFailed ? "failed" : "generated" }).eq("id", packageRunId);

    return NextResponse.json({ ok: true, packageRunId, results });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message || "sba_package_generate_failed" }, { status: 500 });
  }
}

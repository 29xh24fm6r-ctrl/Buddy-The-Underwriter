...
export async function GET(
  ...
  const attachmentIds = Array.from(
    new Set(citations.map((c) => String(c.attachment_id)).filter(Boolean)),
  );
  ...
  const pm = await sb
    ...
    .select(
      "attachment_id, page_number, global_char_start, global_char_end, page_text",
    )
    .eq("deal_id", dealId);
  ...
  const resolved = citations.map((c) => {
    ...
    const gStart =
      c.global_char_start != null ? Number(c.global_char_start) : 0;
    ...
    if ((!pageNum || pStart == null || pEnd == null) && gEnd > gStart) {
      const match = pageMap.find(
        (p) =>
          ...
          gStart >= p.global_char_start &&
          gStart < p.global_char_end,
      );
      if (match) {
        ...
        pEnd = Math.max(
          pStart,
          Math.min(match.page_text_len, gEnd - match.global_char_start),
        );
      }
    }
  ...
  for (const c of resolved) {
    if (!c.page_number || c.page_char_start == null || c.page_char_end == null)
      continue;
    ...
    const words = await sb
      ...
      .select(
        "page_number, word_index, x1, y1, x2, y2, page_char_start, page_char_end",
      )
      .eq("deal_id", dealId)

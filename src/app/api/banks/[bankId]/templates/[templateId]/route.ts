...
export async function PATCH(req: Request, ctx: { params: Promise<{ bankId: string; templateId: string }> }) {
  ...
  if (body?.doc_type !== undefined) patch.doc_type = body.doc_type ? String(body.doc_type).trim() : null;

  if (body?.year_mode != null) {
    const ym = String(body.year_mode).trim();
    if (!["optional", "required", "forbidden"].includes(ym)) {
      return NextResponse.json({ error: "year_mode must be optional|required|forbidden" }, { status: 400 });
    }
    patch.year_mode = ym;
  }
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ bankId: string; templateId: string }> }) {
  const { bankId, templateId } = await ctx.params;
  const sb = supabaseAdmin();

  const { error } = await sb.from("borrower_request_templates").delete().eq("id", templateId).eq("bank_id", bankId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

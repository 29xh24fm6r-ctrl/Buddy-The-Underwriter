...
export async function GET(req: Request, ctx: { params: Promise<{ dealId: string }> }) {
  try {
    ...
    const [{ data: statusRow }, checklist, receipts] = await Promise.all([
      sb.from("deal_portal_status").select("stage, eta_text, updated_at").eq("deal_id", dealId).maybeSingle(),
    ...
    try {
      ...
      if ((d as any)?.borrower_name) display.borrowerName = (d as any).borrower_name;
    } catch {
      // keep defaults
    }

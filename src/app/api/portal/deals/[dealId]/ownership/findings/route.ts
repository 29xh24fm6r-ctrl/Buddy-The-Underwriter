  ...
  try {
    ...
    const proposedCards = (findings ?? []).map((f: any) => ({
      ...
      evidenceSnippet: f.evidence_snippet
        ? String(f.evidence_snippet).slice(0, 120)
        : null,
    }));
    ...
    const confirmedCards = (owners ?? []).map((o: any) => ({
      ...
      ownershipPercent: o.ownership_percent
        ? Number(o.ownership_percent)
        : null,
      requiresPersonalPackage: Boolean(o.requires_personal_package),
    }));

    // Calculate coverage
    const totalPercent = confirmedCards.reduce(
      (sum, c) => sum + (c.ownershipPercent ?? 0),
      0,
    );
    const coverage =
      totalPercent >= 80

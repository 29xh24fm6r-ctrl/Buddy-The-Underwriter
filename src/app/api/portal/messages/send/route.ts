...
export async function POST(req: Request) {
  ...
  if (!body || typeof body !== "string" || !body.trim()) return NextResponse.json({ error: "Missing message" }, { status: 400 });
  ...
  const { error } = await sb.from("borrower_messages").insert({
    ...
    invite_id: invite.id,
    direction: "borrower",
    author_name: typeof authorName === "string" && authorName.trim() ? authorName.trim() : (invite.name || "Borrower"),
    body: body.trim(),
  });

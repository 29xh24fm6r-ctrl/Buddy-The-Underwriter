...
import { viewModelToRenderedSpread } from "@/lib/modelEngine/renderer/viewModelToRenderedSpread";
...
export default async function StandardPrintPage(props: {
  ...
  // V2 pipeline: facts → model → viewModel → adapter → print HTML
  const model = buildFinancialModel(dealId, (facts ?? []) as FinancialFact[]);
  const viewModel = renderFromFinancialModel(model, dealId);
  const rendered = viewModelToRenderedSpread(viewModel);

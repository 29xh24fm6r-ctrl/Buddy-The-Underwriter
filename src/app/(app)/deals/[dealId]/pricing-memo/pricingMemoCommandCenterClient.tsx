...
export default function PricingMemoCommandCenterClient({ dealId }: { dealId: string }) {
  ...
  const loadSnapshots = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/deals/${dealId}/snapshots`);
  ...
  const loadState = useCallback(
    async (snapshotId: string) => {
      ...
      try {
        ...
        if (!res.ok || data?.ok === false) {
          throw new Error(data?.error ?? "Failed to load pricing memo state");
        }
        ...
        console.error(e);
        alert("Failed to load pricing/memo state");
      } finally {
  ...
  const handleNewSnapshot = async () => {
    ...
    try {
      const res = await fetch(`/api/deals/${dealId}/snapshots`, { method: "POST" });
      const data = await res.json();
      ...
      const created: Snapshot | null = data.snapshot ?? null;
      await loadSnapshots();
      ...
      // New snapshot invalidates derived state
      setRiskFacts(null);
  ...
  return (
    <div className="p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-white">Pricing + Memo Command Center</h1>
          <p className="mt-1 text-sm text-white/70">
            Snapshot → Risk Facts → Pricing Quote → Credit Memo → PDF
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-white/10 bg-black/30 p-4">
            <div className="flex items-center justify-between">
              ...
              {loadingState && <div className="text-xs text-white/60">Loading…</div>}
            </div>
        ...
        <div className="space-y-8">
          <RiskFactsCard
            ...
            riskFacts={riskFacts as any}
            onGenerated={(facts) => {
              setRiskFacts(facts as any);
              // Regenerated facts typically invalidates downstream artifacts
              setPricingQuote(null);
              setDocuments([]);
            }}
          />
          ...
          <PricingQuoteEditor
            ...
            quote={pricingQuote as any}
            onCreated={(q) => setPricingQuote(q as any)}
            onUpdated={(q) => setPricingQuote(q as any)}
          />

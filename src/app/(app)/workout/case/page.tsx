    ...
    <div className="min-h-screen bg-[#0f1115] text-white">
      <header className="border-b border-white/10 bg-[#111418] px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Workout Case Management</h1>
            <p className="text-sm text-white/60 mt-1">Special assets and risk mitigation</p>
          </div>
          <Link href="/workout-committee" className="px-4 py-2 bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg transition-colors">
            Workout Center
          </Link>
        </div>
      </header>
        ...
        <div className="bg-[#181b21] border border-white/10 rounded-lg overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/10">
              {deals.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-10 text-center text-white/50">No workout deals found.</td>
                </tr>
              )}
              {deals.map((deal) => (

 ...
 *
 * Single source of truth for the research-quality / committee-readiness payload.
 * Extracted verbatim from the GET /research/quality handler so the credit memo
 * consumes the SAME committee intelligence the Committee Readiness panel shows —
 * no separate, weaker re-derivation.
 *
 * Pure orchestration over the existing pure builders. Read + idempotent task
 * persistence only (unchanged from the route): never mutates gate scoring,
 * committee eligibility, or `committee_grade_accepted`. The route is now a thin
 * wrapper that JSON-serializes `payload`; the memo loader maps `payload` into a
 * ResearchGateSnapshot and `sourceSnapshots` into memo exhibits.
 */

/** Collected source-snapshot row, surfaced for memo exhibit/source coverage. */
export interface ResearchSourceSnapshotRow {
...
export interface ResearchQualityGateView {
  trust_grade: unknown;
  gate_passed: unknown;
  quality_score: unknown;
  evaluated_at: unknown;
  entity_lock_check: unknown;
  entity_confidence: unknown;
  thread_coverage_check: unknown;
  threads_succeeded: unknown;
  threads_failed: unknown;
  source_diversity_check: unknown;
  source_count: unknown;
  management_validation_check: unknown;
  principals_confirmed: unknown;
  principals_unconfirmed: unknown;
  synthesis_check: unknown;
  contradictions_found: unknown;
  underwriting_questions_found: unknown;
  gate_failures: unknown;
  thread_results: unknown;
  section_source_statuses: unknown;
  contradiction_checklist: unknown;
  evidence_quality: unknown;
  preliminary_eligible: boolean;
  committee_eligible: boolean;
  preliminary_basis: unknown;
  committee_blockers: unknown;
}

export interface ResearchQualityPayload {
  ok: true;
  committee_blocker_resolutions: any[];
  committee_requirements_plan: CommitteeRequirementsPlan | null;
  committee_source_candidates: SourceCandidatePlan | null;
  committee_blocker_impact_preview: CommitteeBlockerImpactPreview | null;
  committee_transition_result: CommitteeReadinessTransitionResult | null;
  committee_readiness_section: CommitteeReadinessSection | null;
  committee_decision_evidence: DecisionEvidenceProjection | null;
  research_fact_projection: ResearchFactProjection | null;
  committee_source_collection_plan: CommitteeSourceCollectionPlan | null;
  gate: ResearchQualityGateView | null;
  evidence_summary: {
    total_claims: number;
    by_thread: Record<string, { total: number; fact: number; inference: number; narrative: number }>;
  };
}

export interface ResearchQualityPayloadResult {
  payload: ResearchQualityPayload;
  /** Collected source snapshots for the deal's latest mission (memo exhibits). */
  sourceSnapshots: ResearchSourceSnapshotRow[];
}

export async function buildResearchQualityPayload(
  sb: SupabaseAdminClient,
  dealId: string,
): Promise<ResearchQualityPayloadResult> {
  ...
  let sourceSnapshots: ResearchSourceSnapshotRow[] = [];
  ...
  if (mission?.id) {
    ...
    const [docsRes, factsRes, storyRes, mgmtRes, loanRes, snapsRes] = await Promise.all([
      ...
      // SPEC-…-PR-B: widen the snapshot select so the memo can render honest
      // exhibit labels (url/title/review state). Extra columns are inert to the
      // plan/projection builders below, which only read source_type + status.
      sb.from("buddy_research_source_snapshots")
        .select("source_url, source_type, status, title, source_title, reviewed_status, credibility_rating, connector_kind")
        .eq("mission_id", mission.id),
    ]);

    sourceSnapshots = ((snapsRes.data as any[]) ?? []).map((s) => ({
      source_url: s.source_url ?? null,
      source_type: s.source_type ?? null,
      status: s.status ?? null,
      title: s.title ?? null,
      source_title: s.source_title ?? null,
      reviewed_status: s.reviewed_status ?? null,
      credibility_rating: s.credibility_rating ?? null,
      connector_kind: s.connector_kind ?? null,
    }));
  ...
  const payload: ResearchQualityPayload = {
    ok: true,
  ...
  return { payload, sourceSnapshots };
}

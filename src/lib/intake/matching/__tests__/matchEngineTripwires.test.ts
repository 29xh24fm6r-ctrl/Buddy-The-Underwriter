...
// ─── 13. MATCHING_ENGINE_VERSION starts with "v1.4" ─────────────────────

test("Tripwire v1.4: MATCHING_ENGINE_VERSION starts with 'v1.4'", () => {
  const src = readMatch("types.ts");
  const match = src.match(/MATCHING_ENGINE_VERSION\s*=\s*"([^"]+)"/);
  assert.ok(match, "MATCHING_ENGINE_VERSION must be defined");
  assert.ok(
    match![1].startsWith("v1.4"),
    `MATCHING_ENGINE_VERSION must start with 'v1.4', got: ${match![1]}`,
  );
});

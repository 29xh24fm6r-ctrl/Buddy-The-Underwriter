...
// ─── 13. MATCHING_ENGINE_VERSION starts with "v1.1" ─────────────────────

test("Tripwire v1.1: MATCHING_ENGINE_VERSION starts with 'v1.1'", () => {
  const src = readMatch("types.ts");
  const match = src.match(/MATCHING_ENGINE_VERSION\s*=\s*"([^"]+)"/);
  assert.ok(match, "MATCHING_ENGINE_VERSION must be defined");
  assert.ok(
    match![1].startsWith("v1.1"),
    `MATCHING_ENGINE_VERSION must start with 'v1.1', got: ${match![1]}`,
  );
});

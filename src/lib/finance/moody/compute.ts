// src/lib/finance/moody/compute.ts
import "server-only";
import type { MoodyMetricValue, NormalizedPnl } from "@/lib/finance/types";
import { MOODY_CATALOG } from "./catalog";
...
export function computeMoodyMetricsFromPnl(pnl: NormalizedPnl): MoodyMetricValue[] {
  ...
  const valuesById: Record<string, MoodyMetricValue> = {};
  ...
  return MOODY_CATALOG.map((c) => valuesById[c.id]).filter(Boolean);
}

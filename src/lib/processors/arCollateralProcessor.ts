...
// ─────────────────────────────────────────────────────────────────────────────
// Parsing — best-effort over OCR table extracts
// ─────────────────────────────────────────────────────────────────────────────
...
export function parseARAgingTable(tablesJson: unknown): CustomerRow[] {
  if (!Array.isArray(tablesJson)) return [];
...
function mapHeaderColumns(header: string[]) {
  ...
  for (let i = 0; i < header.length; i++) {
    ...
    if (idx.customer === -1 && /(customer|client|account\s*name|debtor)/.test(s)) {
      ...
      idx.d90 = i;
    } else if (idx.d120 === -1 && /(over\s*90|>\s*90|91\+|120\+|over\s*120|>\s*120|^120(\s*days?)?$)/.test(s)) {
      idx.d120 = i;

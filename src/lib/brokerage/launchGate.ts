/**
 * BRK-10J Launch Gate — master go/no-go command.
 */
...
export type LaunchOptions = { skipBuild?: boolean; strict?: boolean; gate?: string; sb?: any; dbData?: Record<string, any> };
function lg(n: string, c: string, s: LaunchGateStatus, d: number, cr: number, w: number, det: string, r: string[] = []): LaunchGateResult { return { name: n, category: c, status: s, duration: d, critical: cr, warning: w, details: det, repairs: r }; }
...
export function checkRequiredScripts(): LaunchGateResult { const s=Date.now(); let w=0; const r: string[]=[]; for (const f of ["scripts/golden-brokerage-run.ts","scripts/brokerage-integrity-sweep.ts","scripts/brokerage-race-harness.ts","scripts/brokerage-security-audit.ts","scripts/brokerage-business-readiness.ts","scripts/brokerage-compliance-check.ts","scripts/brokerage-revenue-check.ts","scripts/brokerage-launch-gate.ts"]) if (!fe(f)) {w++;r.push(`Create ${f}`);} return lg("required_scripts","operations",w>0?"warn":"pass",Date.now()-s,0,w,w===0?"All present":`${w} missing`,r); }

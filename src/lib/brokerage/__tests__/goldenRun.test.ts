...
test("full run completes",async()=>{const db=new GS();const r=await runGoldenBrokerageRun({sb:db as any,brokerageBankId:"brk-1"});assert.equal(r.ok,true,`Failed at ${r.failedStage}: ${r.failedReason}`);assert.ok(r.dealId);assert.ok(r.listingId);assert.ok(r.claimId);assert.ok(r.pickId);assert.ok(r.accessId);assert.equal(r.score,78);assert.equal(r.band,"strong_fit");assert.equal(r.lenderName,"Golden Test Bank");assert.equal(r.evidenceClass,"synthetic_direct_insert");assert.equal(r.commissionsGoldenTrident,false);assert.equal(db.tables.deals.length,1);assert.equal(db.tables.marketplace_package_access.length,1);});
test("every synthetic financial fact carries the production-required period",async()=>{const db=new GS();const r=await runGoldenBrokerageRun({sb:db as any,brokerageBankId:"brk-1"});assert.equal(r.ok,true);assert.ok(db.tables.deal_financial_facts.length>0);for(const fact of db.tables.deal_financial_facts){assert.equal(fact.fact_period_start,"2025-01-01");assert.equal(fact.fact_period_end,"2025-12-31");}});
...
test("reuses and preserves the dedicated sandbox lender fixture",async()=>{const db=new GS();db.tables.banks.push({id:"golden-lender",code:"GOLDEN_TEST_FIXTURE",name:"Golden Test Bank",bank_kind:"commercial_bank",is_sandbox:true});db.tables.lender_marketplace_agreements.push({id:"golden-agreement",lender_bank_id:"golden-lender",status:"active"});const r=await runGoldenBrokerageRun({sb:db as any,brokerageBankId:"brk-1",cleanup:true});assert.equal(r.ok,true,`${r.failedStage}: ${r.failedReason}`);assert.equal(r.lenderBankId,"golden-lender");assert.equal(db.tables.deals.length,0);assert.equal(db.tables.banks.length,1);assert.equal(db.tables.lender_marketplace_agreements.length,1);});

...
test("lender summary uses the exact authorized listing and refuses a mismatched seal", async()=>{
  const db=sealedDb();
  db.tables.marketplace_listings.unshift({id:"historical",sealed_package_id:"other",deal_id:"d1",loan_amount:1});
  const result=await m.getLenderPackageAccess("acc1","b1",db as any);
  assert.equal(result.ok,true);
  if(result.ok) assert.equal(result.access.dealSummary.loanAmount,850000);
  db.tables.marketplace_listings[1].sealed_package_id="different-seal";
  assert.equal((await m.getLenderPackageAccess("acc1","b1",db as any)).ok,false);
});

...
function fixture(): Parameters<typeof assertPackageFinancialLineage>[0] {
  ...
  return {
    snapshot: { id: "financial-1", output: { spreadInput: {}, projectionModel: { annualProjections: annual, monthlyProjections: monthly }, baseYear: base, sourcesAndUses: uses, balanceSheetProjections: balance } } as any,
    ...
    feasibility: { projections_package_id: "package-1" }, memo: { metadata_json: { financial_snapshot_id: "financial-1", financial_payload: { baseYear: base, annualProjections: annual, sourcesAndUses: uses, balanceSheetProjections: balance } } }, spread: { rendered_json: { financialSnapshotId: "financial-1", financialRenderInputHash: deterministicHash({}) } },
  };
}

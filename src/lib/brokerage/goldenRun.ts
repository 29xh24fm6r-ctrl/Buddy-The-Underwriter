...
async function s1(c: Ctx): Promise<StepResult> {
  const id = uuid();
  const { error } = await c.sb.from("deals").insert({ id, bank_id: c.brokerageBankId, deal_type: "SBA", origin: "brokerage_anonymous", display_name: NAME, borrower_name: "Golden Borrower", borrower_email: "golden@test.local", loan_amount: AMOUNT, state: "TX", status: "active", metadata: { golden_test: true } });
...
async function s3(c: Ctx): Promise<StepResult> {
  const { error } = await c.sb.from("deal_borrower_story").upsert({ deal_id: c.dealId, business_description: "Regional metal fabrication shop.", products_services: "CNC machining, welding.", customers: "Construction contractors.", growth_strategy: "New 5-axis CNC.", key_risks: "Steel price volatility.", source: "borrower_self", confidence: 0.95 }, { onConflict: "deal_id" });
...
async function s5(c: Ctx): Promise<StepResult> {
  const { error } = await c.sb.from("buddy_sba_scores").insert({ id: uuid(), deal_id: c.dealId, bank_id: c.brokerageBankId, score_status: "locked", score: SCORE, band: BAND, rate_card_tier: "standard", eligibility_passed: true, component_scores: { borrower_strength: 4, business_strength: 3.8, deal_structure: 3.5, repayment_capacity: 4.2 }, top_strengths: ["8 years", "Clear proceeds"], top_weaknesses: ["Single location"], narrative: "Score 78.", computed_at: now(), metadata: { golden_test: true } });
...
async function s6(c: Ctx): Promise<StepResult> {
  const { error } = await c.sb.from("buddy_trident_bundles").insert({ id: uuid(), deal_id: c.dealId, bank_id: c.brokerageBankId, mode: "preview", status: "succeeded", version: 1, generation_started_at: now(), generation_completed_at: now(), redactor_version: "1.0.0", business_plan_pdf_path: `golden/${c.dealId}/bp.pdf`, projections_pdf_path: `golden/${c.dealId}/proj.pdf`, feasibility_pdf_path: `golden/${c.dealId}/feas.pdf`, metadata: { golden_test: true, stubbed: true } });
...
async function s9(c: Ctx): Promise<StepResult> {
  ...
  if (!ex) { const { error } = await c.sb.from("banks").insert({ id: c.lenderBankId, code: "GOLDEN_TEST_BANK", name: LENDER, bank_kind: "commercial_bank", active: true }); if (error && !error.message?.includes("duplicate")) return { ok: false, error: `bank: ${error.message}` }; }
...
async function s10(c: Ctx): Promise<StepResult> {
  ...
  if (error) {
    ...
    const cid = uuid();
    const { error: e2 } = await c.sb.from("marketplace_claims").insert({ id: cid, listing_id: c.listingId, deal_id: c.dealId, lender_bank_id: c.lenderBankId, status: "active", claimed_at: now(), committed_rate_bps: RATE });
...
async function s11(c: Ctx): Promise<StepResult> {
  ...
  await c.sb.from("marketplace_claims").update({ status: "picked", updated_at: ts }).eq("id", c.claimId);
  await c.sb.from("marketplace_claims").update({ status: "rejected", expired_at: ts, updated_at: ts }).eq("listing_id", c.listingId).eq("status", "active").neq("id", c.claimId);
...
const STEPS: Array<{ name: string; fn: (c: Ctx) => Promise<StepResult> }> = [
  ...
  { name: "seal", fn: s7 }, { name: "listing", fn: s8 },
  { name: "lender_agreement", fn: s9 }, { name: "claim", fn: s10 },
...
export async function cleanupGoldenRun(sb: any, dealId: string): Promise<void> {
  for (const t of ["marketplace_package_access", "marketplace_audit_log", "marketplace_picks", "marketplace_claims", "marketplace_listings", "buddy_sealed_packages", "buddy_trident_bundles", "buddy_sba_scores", "deal_financial_facts", "deal_documents", "deal_borrower_story", "borrower_applications", "borrower_concierge_sessions"]) {

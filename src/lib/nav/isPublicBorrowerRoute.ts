 ...
 * (/portal/owner/[token], /portal/share/[token], and the borrower
 * magic-link /portal/[token] handled by isPublicBorrowerPortalRoute below)
 * are exempted.
 */
...
export function isPublicBorrowerRoute(
  ...
  if (isPublicBorrowerPortalRoute.test(pathname)) return true;

 ...
 *
 * SPEC-M6 ANTICIPATED-INTERROGATION-1 adds a fifth source: unanswered,
 * borrower-resolvable hostile-committee questions (deal_hostile_interrogations,
 * generated by src/lib/brokerage/hostileInterrogation.ts). These map
 * directly to FixCard — no detectFixCardIssues/fixCardCopyCache round trip
 * needed, since the verifier that generated the question already produced
 * deal-specific rationale/resolving-action text in the same pass (unlike
 * the other four sources' generic, category-cached copy).
 */
...
export async function buildFixCards(dealId: string, sb: SB): Promise<FixCard[]> {
  ...
  // SPEC-M6 ANTICIPATED-INTERROGATION-1 — see module doc comment.
  const { data: hostileRows } = await sb
    .from("deal_hostile_interrogations")
    .select("code, question, severity, rationale, resolving_action")
    .eq("deal_id", dealId)
    .eq("already_answered", false)
    .eq("borrower_resolvable", true);

  for (const row of (hostileRows ?? []) as any[]) {
    cards.push({
      issueType: `hostile_qna:${row.code}`,
      severity: row.severity as FixCardSeverity,
      what: row.question,
      whyItMatters: row.rationale,
      resolvingAction: row.resolving_action,
    });
  }

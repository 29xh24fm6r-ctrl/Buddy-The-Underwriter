...
/**
 * buildCreditMemoPdf — PDFKit renderer for the certified Florida Armory
 * credit memo.
 *
 * Extracted from src/app/api/deals/[dealId]/credit-memo/canonical/pdf/route.ts
 * so it can be reused by other committee-safe render paths (e.g. the
 * brokerage credit-memo download route) without a second, divergent PDF
 * renderer. This module renders only — callers are responsible for loading
 * a certified snapshot (loadLatestCertifiedFloridaArmorySnapshot) and
 * running assertCommitteeMemoSafe() before calling this function.
 */
...
import type { StressScenarioRow } from "@/lib/creditMemo/canonical/buildStressTestTable";

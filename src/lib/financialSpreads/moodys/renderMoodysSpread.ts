...
/**
 * Render the Moody's Financial Analysis package.
 *
 * Returns a RenderedSpread-compatible structure with multi-period support.
 */
export function renderMoodysSpread(input: MoodysRenderInput): RenderedSpread {
  ...
  let lastStatement: MoodysStatement | null = null;
...
export function renderMoodysSpreadWithValidation(
  ...
  if (!input.snapshot) {
    return renderMoodysSpread(input);
  }
...
function formatStatementLabel(statement: MoodysStatement): string {
  switch (statement) {

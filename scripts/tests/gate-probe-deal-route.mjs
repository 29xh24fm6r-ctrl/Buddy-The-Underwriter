#!/usr/bin/env node
/**
 * Gate Probe: Deal Route API Shape Validation
 *
 * Prevents regression by validating that GET /api/deals/:id
 * returns the expected JSON shape and NEVER returns 500.
 *
 * Usage:
 *   node scripts/tests/gate-probe-deal-route.mjs [baseUrl] [dealId]
 *
 * Environment:
...
console.log("🔍 Gate Probe: Deal Route API Shape Validation");
console.log(`   Base URL: ${BASE_URL}`);
console.log(`   Deal ID: ${DEAL_ID}`);
console.log("");

/**
 * Assert the deal response shape is valid.
 */
function assertDealResponseShape(data, context) {
  ...
  assert.ok(
    typeof data.deal === "object" && data.deal !== null,
    `${prefix} response.deal should be an object, got: ${typeof data.deal}`
  );
  ...
  // Deal object must have at least id
  assert.ok(
    typeof data.deal.id === "string" || data.deal.id === null,
    `${prefix} deal.id should be a string or null, got: ${typeof data.deal.id}`
  );
  ...
  if (!data.ok) {
    ...
    // correlationId is in meta, not error - verify meta.correlationId exists
    assert.ok(
      typeof data.meta?.correlationId === "string",
      `${prefix} meta.correlationId should be a string when ok=false, got: ${typeof data.meta?.correlationId}`
    );
  }

  console.log(`   ✅ ${prefix} Shape valid: ok=${data.ok}, deal.id=${data.deal.id}`);
}

async function runProbe() {
  const url = `${BASE_URL}/api/deals/${DEAL_ID}`;
  ...
  if (response.status === 500) {
    ...
    console.error("   The deal route API must NEVER return 500.");
  ...
  if (response.status === 200) {
    assertDealResponseShape(data, data.ok ? "200 OK" : "200 Error");
  } else {
  ...
  console.log("");
  console.log("✅ Gate probe passed: Deal Route API shape is valid");

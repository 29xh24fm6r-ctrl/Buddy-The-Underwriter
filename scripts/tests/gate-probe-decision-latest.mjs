#!/usr/bin/env node
/**
 * Gate Probe: Decision Latest API Shape Validation
 *
 * Prevents regression by validating that GET /api/deals/:id/decision/latest
 * returns the expected JSON shape and NEVER returns 500.
 *
 * Usage:
 *   node scripts/tests/gate-probe-decision-latest.mjs [baseUrl] [dealId]
 *
 * Environment:
...
console.log("🔍 Gate Probe: Decision Latest API Shape Validation");
console.log(`   Base URL: ${BASE_URL}`);
console.log(`   Deal ID: ${DEAL_ID}`);
console.log("");

/**
 * Assert the decision latest response shape is valid.
 */
function assertResponseShape(data, context) {
  ...
  console.log(`   ✅ ${prefix} Shape valid: ok=${data.ok}, snapshot=${data.snapshot ? "present" : "null"}`);
}

async function runProbe() {
  const url = `${BASE_URL}/api/deals/${DEAL_ID}/decision/latest`;
  ...
  if (response.status === 500) {
    ...
    console.error("   The decision/latest API must NEVER return 500.");
  ...
  console.log("");
  console.log("✅ Gate probe passed: Decision Latest API shape is valid");
